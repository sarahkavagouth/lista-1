...
INSERT INTO Livros (titulo, autor, ano_publicacao, disponivel, categoria, isbn, editora, quantidade_paginas, idioma)
VALUES ('Por trás das estrelas', 'Apollo Novaes', 2019, TRUE, 'História', '999-0123456789', 'Editora Espacial', 150, 'Português');

UPDATE Livros 
SET Disponivel = FALSE WHERE Ano < 2000;
...
DELETE FROM Livros 
WHERE Idioma = 'Francês' AND Ano < 1970;
...
SELECT Categoria, COUNT(*) AS Total FROM Livros 
GROUP BY Categoria;
...
AVG(Paginas) AS Media FROM Livros 
WHERE Categoria = 'Drama';

SELECT AVG(Ano) AS MediaAno FROM Livros 
WHERE Disponivel = TRUE;

SELECT MAX(Ano) AS Recente, 
MIN(Ano) AS Antigo FROM Livros;
